...
def get_parameters(to_dict=False, config=None, **new_kwargs):
    ...
    # parser.add_argument('-c', '--config', type=str, default=config or ' ')
    # parser.add_argument('--local_rank', type=int, default=0)
    # parser.add_argument('--world_size', type=str, default='1') #1
    # parser.add_argument('--cuda_visible_devices', type=str, default='7')  # 1
    ...
    try:
        oc_cfg = omegaconf.OmegaConf.load(config)
    except Exception as e:
        logger.warning(f'config path [{config}] is not valid.')
        raise e

    # #lmh_change
    os.environ["WORLD_SIZE"] = oc_cfg.world_size
    os.environ["CUDA_VISIBLE_DEVICES"] = oc_cfg.cuda_visible_devices

    if 'WORLD_SIZE' in os.environ:
        args_tmp.distributed = int(os.environ['WORLD_SIZE']) > 1
    if oc_cfg.debug:
        oc_cfg.train.epochs = 2 #lmh_change: args_tmp
